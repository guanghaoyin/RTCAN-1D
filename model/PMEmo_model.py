...
from model.PMEmo_Resnet1d import *
import torch
import torch.nn as nn


class Model(nn.Module):
    def __init__(self, use_music = True, use_attention = False, use_non_local = False, input_channel = 3, num_class = 2):
        ...
        if use_music:
            self.music_vector_dimension = 6373
        ...
        if self.use_res_sub:
            ...
            self.Res_sub = ResNet1D(self.inter_channel, self.use_res_sub, self.use_attention)

        else:
            self.Res_sub = ResNet1D(self.input_channel, self.use_res_sub)
            self.eda_vector_dimension = 512

        if use_music:
            self.regression = self.Regression_add()
        else:
            self.regression = self.Regression()
    def forward(self, eda, music_vector):
        if self.use_res_sub:
        ...
        if self.use_music:
            vector = torch.cat((eda_vector, music_vector), 1)
        else:
            vector = eda_vector
        out = self.regression(vector)
        return out

    def Regression_add(self):
        return nn.Sequential(
            nn.Linear(self.eda_vector_dimension + self.music_vector_dimension, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, self.output_channel),
            nn.Softmax() if self.use_softmax else nn.Sigmoid()
        )

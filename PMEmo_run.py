from model.PMEmo_model import *
from utils import timer, visual_loss
from data.PMEmo.data_loader_1D import *
...
def predict(model, data, vector, label, total, correct):
    n0_true = 0
    n0_false = 0
    n1_true = 0
    n1_false = 0
...
def train(model, criterion, optimizer, data, vector, label,reg_loss):
    data = data.to(device)
    label = label.to(device)
    vector = vector.to(device)
...
def run_train(type, cvx, use_music = True, use_attention = True, use_non_local = True, load_model =False):
    #create data_loader
    data_loader = data_loader_1D(type, cvx, fold_cross_num)

    #define model_name
    model_name = 'Resnet1d_' + type + '_'
    if use_music: model_name += 'M'
    ...
    model = Model(use_music, use_attention, use_non_local, num_class=2)
    if load_model:
        model.load_state_dict(torch.load('./save/' + model_name + '.pt')).to(device)
    else:
    ...
    for fold_num in range(1):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (dataId, target) in enumerate(train_loader):
                ...
                vector = torch.Tensor(get_batch_vector(dataId, data_loader))
                loss, outputs = train(model, criterion, optimizer, data, vector, target,reg_loss)
        ...
        #evaluate each batch
            with torch.no_grad():
                ...
                for batch_idx, (dataId, target) in enumerate(test_loader):
                    ...
                    data = torch.Tensor(get_batch_data(dataId, data_loader))
                    vector = torch.Tensor(get_batch_vector(dataId, data_loader))
                    total, correct, a, b, c, d = predict(model, data, vector, target, total, correct)
                    # loss = criterion1()
                    record['n0_true'] += a
                    record['n0_false'] += b
                    record['n1_true'] += c
            ...
            with open('./save/loss/' + visualizer_env + '_test accuracy.txt', 'w') as f:
...
if __name__ == '__main__':
    run_train('Arousal','tonic', use_music = True, use_attention = True, use_non_local = True)
    run_train('Valence','tonic', use_music = True, use_attention = False, use_non_local = True)
